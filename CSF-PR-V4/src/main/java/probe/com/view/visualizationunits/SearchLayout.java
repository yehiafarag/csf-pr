...
public class SearchLayout extends VerticalLayout implements Serializable, Button.ClickListener {
    ...
    private TreeMap<Integer, String> datasetNamesList;
    private Map<Integer, DatasetBean> datasetList = null;
        ...
        if (protSearchObject == null || protSearchObject.toString().equals("") || protSearchObject.toString().length() < 4 || protSearchObject.toString().equals("Please use one key-word per line and choose the search options")) {
            ...
            if (searchMethod.equals("Protein Accession"))//case of protein accession
            {
                for (String searchStr : searchSet) {
                    searchProtList = handler.searchProteinByAccession(searchStr.trim(), datasetList, searchDatasetType,validatedOnly);
            ...
            {
                for (String searchStr : searchSet) {
                    searchProtList = handler.searchProteinByName(searchStr.trim(), datasetList, searchDatasetType,validatedOnly);
                    if (searchProtList == null || searchProtList.isEmpty()) {
                        notFound += searchStr + "\t";
                    } else {
                        fullExpProtList.putAll(searchProtList);
                ...
                pepSet = searchSet;
                for (String searchStr : searchSet) {
                    searchProtList = handler.searchProteinByPeptideSequence(searchStr.trim(), datasetList, searchDatasetType, validatedOnly);
                    if (searchProtList == null || searchProtList.isEmpty()) {
                        notFound += searchStr + "\t";
                    } else {
                        fullExpProtList.putAll(searchProtList);
            ...
            if (fullExpProtList == null || fullExpProtList.isEmpty()) {
                ...
                final SearchResultsTableLayout searcheResultsTableLayout = new SearchResultsTableLayout(handler, datasetList, fullExpProtList,validatedOnly);
                ...
                Property.ValueChangeListener listener = new Property.ValueChangeListener() {
                    ...
                    public synchronized void valueChange(Property.ValueChangeEvent event) {
                        ...
                        int expId = util.getExpId(expName, datasetList);
                        DatasetBean exp = datasetList.get(expId);
                        proteinsList = handler.getProteinsList(exp.getExpId(), datasetList);
                        exp.setProteinList(proteinsList);
                        datasetList.put(exp.getExpId(), exp);
                        CustomExportBtnLayout ce1 = new CustomExportBtnLayout(handler, "allProtPep", expId, expName, accession, otherAccession, datasetList, null, 0, null, null, null);
...
//                        PopupView p1 = new PopupView("Export All (" + accession + ")'s Peptides", ce1);
                        searcheResultsTableLayout.setExpBtnProtAllPepTable(p1);// new PopupView("Export Proteins", (new CustomExportBtnLayout(handler, "prots",expId, expName, accession, otherAccession, datasetList, proteinsList, exp.getFractionsNumber(), null,null))));
                        ...
                        if (key < 0) {
                            ...
                            if (pepProtList.isEmpty()) {
                                ...
                                CustomExportBtnLayout ce3 = new CustomExportBtnLayout(handler, "protPep", exp.getExpId(), exp.getName(), accession, otherAccession, datasetList, null, 0, pepProtList, null, null);
                            ...
                            fractionsList = handler.getFractionsList(exp.getExpId(), datasetList);
