...
public class FileValidator implements Serializable {
    ...
    public int validateFile(String[] strArr, String MIMEType)//check if the file type and  file format
    {
        ...
        if (!MIMEType.equalsIgnoreCase("text/plain") && (!MIMEType.equalsIgnoreCase("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")) && strArr.length < 3)//the file type must be 'txt' and the rows greater than 8 (in fraction protein initial rows is 8) 
        {
        ...
        } else if (MIMEType.equalsIgnoreCase("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") && strArr.length == 5 && strArr[0].equalsIgnoreCase("MW  (kDa)") && strArr[1].equalsIgnoreCase("Name") && strArr[2].contains("Lower")) {
