...
public class SearchUnit extends CustomComponent implements ClickListener,Serializable{
					...
					protExpTab.addListener(new Property.ValueChangeListener(){
						...
						if(peptideLayout != null){
							...
							else
							{
								...
								if(pepLabel != null)
									peptideLayout.removeComponent(pepLabel);
								if(pepSearch == 1){pepTable  = new PeptideTable(pepProExpList,pepSet);}
								else{pepTable  = new PeptideTable(pepProExpList,null);}
							...
							/// fraction part
							 ...
							 if(exp.getFractionRange() == 0)
							    ...
							    Map<Double,ProteinBean> proteinFractionAvgList = eh.getProteinFractionAvgList(accession, exp.getFractionsList(),exp.getExpId());
							   ...
							   fractionPlotView = new FractionsPlots(proteinFractionAvgList,mw,ranges);
	...
	@SuppressWarnings("deprecation")
	public Table getFractionTable(Map<Double, ProteinBean> proteinFractionAvgList,ArrayList<String> ranges)
	{
