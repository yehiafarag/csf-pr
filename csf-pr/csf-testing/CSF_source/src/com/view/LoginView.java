...
public class LoginView extends VerticalLayout implements  TabSheet.SelectedTabChangeListener,Serializable, Button.ClickListener{
    ...
    private  Panel root1;
    private  Panel root2;
    private VerticalLayout changePasswordPanel;
	...
	private Label removeExperimentLabel;
	private String url, dbName, driver, userName,  password;
    
    public LoginView(String url,String dbName,String driver,String userName, String password) {
	...
	public void buttonClick(ClickEvent event) {//on click login button
		...
		if(valid == false){	//user not valid 
			...
			if(error != null)
			 ...
			 error = new Label("<h4 style='color:red'>Please Check Your Username or Password ! </h4>");
	...
	private void initRoot1()//login form 
	{
		if(signOutButton != null)
	     ...
	     Label l = new Label("<h4 style='color:blue'>Login To Be Able To Add/Edit Experiments </h4>");
	     l.setContentMode(Label.CONTENT_XHTML);	 root1.addComponent(l); 
	     // Create the Form
	     ...
	     Label l2 = new Label("<h4 style='color:blue'>For Sign Up Please Contact Us at admin@csf.com  </h4>");
	     l2.setContentMode(Label.CONTENT_XHTML);	           
	     root1.addComponent(l2);
	...
	private VerticalLayout initAdminLayout()//admin layout
	{
	    ...
	    newRegForm.addField(Integer.valueOf(1), usernameField);
	    newRegForm.addField(Integer.valueOf(2), passwordField);
	    newRegForm.addField(Integer.valueOf(3), rePasswordField);
	    newRegForm.addField(Integer.valueOf(4), regButton);
	    regButton.addListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				{
						valid = auth.reg(username.toUpperCase(),password,false);								
				}
				if(valid == false){	
				
					if(error != null)
						adminLayout.removeComponent(error);
					 error = new Label("<h4 style='color:red'>Failed to Register The User Please Check The User Username, Password,  Re-Password ! </h4>");
					 error.setContentMode(Label.CONTENT_XHTML);
	...
	private void updateRightSide()//Handle remove user by admin
	{
	    ...
	    removeExperimentLabel = new Label("<h4 style='color:blue'>Remove Existing Experiment</h4>");
	    removeExperimentLabel.setContentMode(Label.CONTENT_XHTML);
	    removeExperimentLabel.setWidth("50%");
		
	    rightSide.addComponent(removeExperimentLabel);
		rightSide.setComponentAlignment(removeExperimentLabel, Alignment.MIDDLE_CENTER);//(removeUserLabel, ALIGNMENT_VERTICAL_CENTER, ALIGNMENT_TOP);
    	
		if(removeExperimentForm != null)
			rightSide.removeComponent(removeExperimentForm);
		removeExperimentForm = new Form(); 
		removeExperimentForm.setWidth("50%");
		expList = eh.getExperiments(null);
	    selectExp = new Select("Experiment ID", expList.keySet());		
	    ...
	    removeExpButton.addListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				Integer expId = (Integer)selectExp.getValue();
				if(expId != null)
				{
	    ...
	    rightSide.addComponent(removeExperimentForm);
	    rightSide.setComponentAlignment(removeExperimentForm, Alignment.MIDDLE_CENTER);	
	...
	private void initLayout1() {
		if(layout1 != null)
			layout1.removeAllComponents();
    	UploadUnit uploader = new UploadUnit(url,dbName,driver,userName, password);
