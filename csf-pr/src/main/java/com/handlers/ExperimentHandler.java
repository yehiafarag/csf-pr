...
public class ExperimentHandler implements Serializable{
	...
	public Map<Double,ProteinBean> getProteinFractionAvgList( String accession,Map<Integer, FractionBean> fractionsList,int expId)
	{
		Map<Double,ProteinBean> proteinFractList = new TreeMap<Double,ProteinBean>();
			...
			for(int k:treeFractList.keySet())
			{
				FractionBean fb = fractionsList.get(k);
					...
					if(fb.getProteinList().containsKey(accession)){
						proteinFractList.put(fb.getMinRange(), fb.getProteinList().get(accession));
					}
			}
