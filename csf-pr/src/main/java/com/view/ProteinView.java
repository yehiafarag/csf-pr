...
public class ProteinView extends VerticalLayout implements  Serializable,Property.ValueChangeListener{
								...
								// mw.setWidth("100%");
								  ...
								  if(exp != null && exp.getProteinList() != null)
								  {
									  ...
									  double mw = 0.0;
								       ...
								       Map<Double,ProteinBean> proteinFractionAvgList = eh.getProteinFractionAvgList(accession, fractionsList,exp.getExpId());
								        ...
								        FractionsPlots fractionPlotView = new FractionsPlots(proteinFractionAvgList,mw,ranges);
								       ...
								       fractionsDataLayout.addComponent(rangeTable);
								       fractionsDataLayout.setExpandRatio(fractionPlotView, 3.5f);
								       fractionsDataLayout.setExpandRatio(rangeTable, 1.5f);
								       fractionsDataLayout.setComponentAlignment(rangeTable, Alignment.MIDDLE_RIGHT);
	...
	@SuppressWarnings("deprecation")
	private Table getFractionTable(Map<Double, ProteinBean> proteinFractionAvgList, ArrayList<String> ranges)
	{
		 ...
		 for(ProteinBean pb: proteinFractionAvgList.values()){
			 table.addItem(new Object[] { ranges.get(x).split("\t")[1],pb.getNumberOfPeptidePerFraction(),pb.getNumberOfSpectraPerFraction(),pb.getAveragePrecursorIntensityPerFraction()}, new Integer(x+1));	
			 x++;			
		 }
